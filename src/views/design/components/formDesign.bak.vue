              ...
              <ak-tabs :class="[item.className]">
                <ak-tab-pane
                  ...
                  <draggable
                    ...
                    v-bind="{group:'form', ghostClass: 'ghost',animation: 200, handle: '.drag-move'}"
                    @add="_draggableTabsAdd(item, index, $event)">
                    <transition-group name="fade" tag="div" class="tabs-col-list">
                      <template v-for="(pane, paneIndex) in tab.list">
                        <div :key="paneIndex">{{pane.type}}</div>
                      </template>
                    </transition-group>
                  </draggable>
                </ak-tab-pane>
