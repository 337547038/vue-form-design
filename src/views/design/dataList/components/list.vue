...
<script setup lang="ts">
  ...
  const getListData = (page?: number) => {
    ...
    // 当前表单参数，单独包起来防跟查询条件字段重名冲突
    const addParams = props.formId ? { params: { formId: props.formId } } : {}
    const params = {
      pageInfo: {
    ...
    getRequest(getUrl, params)
      .then((res) => {
        state.loading = false
        let formatResult: any = res.data
        const afterResponse = props.tableData.events?.afterResponse
        if (typeof afterResponse === 'function') {
          formatResult = afterResponse(formatResult) ?? formatResult
        }
        if (typeof props.afterResponse === 'function') {
          formatResult = props.afterResponse(formatResult) ?? formatResult
        }
