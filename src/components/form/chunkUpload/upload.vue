...
<template>
  ...
  <div class="split-upload-list">
    ...
    >
      <el-button
        v-if="tempFileList.length&&pageStatus===0"
        type="primary"
        @click="confirmUpload"
      >
        确认上传
...
const startTime = ref()
const chunkSize = 10 * 1024 * 1024 // 每片大小
const maxParallelUploads = 3 // 同时上传并发数

const pageStatus = ref(0) // 页面状态0显示上传按钮其他不显示
...
const axiosUpload = async (currentFile: TempFileList) => {
  ...
  currentFile.fileHash = fileMd5 + '' // 添加一个属性用于在上传列表中删除
  currentFile.source = source
...
const delUploadClick = (index: number) => {
  ...
  if (file.source) {
    file.source.cancel('del')
  }
  // 有可能是已经上传了部分或已完成上传
  if ([1, 2, 3, 4].includes(file.status) && file.fileHash) {
    getRequest('chunkUploadDel', { fileHash: file.fileHash }).then(() => {
