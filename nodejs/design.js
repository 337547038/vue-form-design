...
router.post('/list', async (req, res) => {
  ...
  let where = ' WHERE id is not null order by id desc'
  if (whereTemp.length) {
    where = ` WHERE ${whereTemp.join(' AND ')} order by id desc`
  }
  const sql = `SELECT (SELECT name FROM ${datasource} where id=d.source) as sourceName,source,id,name,status,creatDate,updateDate,category,roleId,icon FROM \`design\` as d ${where} Limit ${start},${pageSize}`
...
router.post('/change', (req, res) => {
  ...
  const changeField = ['name', 'category', 'roleId', 'status']
  const params = {}
