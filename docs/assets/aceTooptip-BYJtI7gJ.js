...
<\/script>`;U(()=>{k.value=q(g,"editJsonCopy","html")})},s=t=>{var R,S,w,L;const c=((R=t.config)==null?void 0:R.openType)==="dialog",d=((S=t.config)==null?void 0:S.dialogWidth)||"600px",y=((w=t.config)==null?void 0:w.requestUrl)||"";y&&delete t.config.requestUrl;const o=((L=t.config)==null?void 0:L.deleteUrl)||"";o&&delete t.config.deleteUrl;let g="",v="",_="";c&&(_='@btn-click="listBtnClick"',g=`<el-dialog
      destroy-on-close
...
<script setup lang="ts">
  ...
  ${d}
<\/script>
${c}`;U(()=>{k.value=q(g,"editJsonCopy","html")})},u=t=>{U(()=>{const c=new ge(t.target,{text:()=>k.value.getValue()});c.on("success",function(){J({message:"复制成功！",type:"success"}),c.destroy()}),c.on("error",function(){J.error("复制失败"),c.destroy()}),c.onClick(t)})},i=()=>{var o;const t="data:text/csv;charset=utf-8,"+((o=k.value)==null?void 0:o.getValue()),c=new Date().getTime()+".vue",d=encodeURI(t),y=document.createElement("a");y.setAttribute("href",d),y.setAttribute("download",c),y.click()};return A({open:m,openTable:s,openScreen:e}),(t,c)=>{const d=I("el-button"),y=I("el-dialog");return N(),G(y,{modelValue:b.value,"onUpdate:modelValue":c[0]||(c[0]=o=>b.value=o),title:"导出vue文件",class:"export-dialog",width:"80%"},{footer:C(()=>[B("div",he,[z(d,{type:"primary",onClick:u},{default:C(()=>[V(" 复制数据 ")]),_:1}),z(d,{type:"primary",onClick:i},{default:C(()=>[V(" 导出代码 ")]),_:1})])]),default:C(()=>[ve]),_:1},8,["modelValue"])}}}),be={class:"main-tools"},we=Y({__name:"headTools",props:{showKey:{default:()=>[]},hideKey:{default:()=>[]}},emits:["click"],setup(x,{emit:A}){const b=x,k=A,h=le(()=>{var e,u;const s=[{icon:"del",label:"清空",key:1},{icon:"eye",label:"预览",key:2},{icon:"json",label:"生成脚本预览",key:3},{icon:"vue",label:"导出vue文件",key:4},{icon:"save",label:"保存",key:5}];return(e=b.showKey)!=null&&e.length?s.filter(i=>b.showKey.includes(i.key)):(u=b.hideKey)!=null&&u.length?s.filter(i=>!b.hideKey.includes(i.key)):s}),m=s=>{k("click",s)};return(s,e)=>{const u=I("el-button");return N(),j("div",be,[ue(s.$slots,"default"),(N(!0),j(fe,null,de(h.value,i=>(N(),G(u,{link:"",type:"primary",onClick:t=>m(i.icon),key:i.icon},{default:C(()=>[B("i",{class:pe(["icon-"+i.icon])},null,2),V(me(i.label),1)]),_:2},1032,["onClick"]))),128))])}}}),xe={after:"获取数据响应处理之后事件，可对请求返回数据进行处理；支持返回字符串形式",before:"获取数据请求发送之前事件，可对请求参数进行处理；支持返回字符串形式",editCss:"当前表单应用页的样式，类似于.vue文件中的style scoped中的样式",change:"表单组件值改变事件。这里可修改其他组件的值。返回字符串形式可在@/utils/formChangeValue中处理",editRules:"可参考UI组件表单校验，<a href='https://element-plus.gitee.io/zh-CN/component/form.html#%E8%A1%A8%E5%8D%95%E6%A0%A1%E9%AA%8C' target='_blank' style='color:red'>详情点击</a>",editProps:"可添加当前组件所有prop属性及事件方法",creatJson:"可编辑修改或将已生成的脚本粘贴进来",button:"可添加当前组件所有prop属性及事件方法"},Se=x=>{switch(x){case"before":return`opt=(params, obj) => {
  // params请求的参数，需对params作修改后return回去。obj其他参数
  return params
}`;case"after":return`opt=(res, success, type) => {
...
}`}};export{we as _,Ee as a,Se as b,xe as g};
