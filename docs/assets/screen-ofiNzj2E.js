/* empty css                                                            */import{o as s,c as a,af as n}from"./index-PaNBskVU.js";const e={class:"marked-body"},l=n(`<h1 id="可视化数据大屏">可视化数据大屏</h1><p>数据可视化大屏使用<code>position</code>和<code>let</code>和<code>top</code>定位布局。</p><p>注意：当使用了<code>right、top</code>或其他非<code>px</code>单位布局时，当前组件将不能进拖动缩放及合并等操作</p><h2 id="大屏配置">大屏配置</h2><p>包括一些基本的属性配置项，设计大屏宽高背景色及大屏全局样式</p><p>其中<code>编辑样式</code>可进行css样式编辑</p><h3 id="全局数据">全局数据</h3><p>当可视化大屏引用组件比较多时，若每个组件都单独从接口获取数据，这种做法明显不合理，后端同学可能也不会这么提供。 因此引入全局数据请求接口，将数据接口处理好提供给每个组件调用。如可在<code>afterFetch</code>中对接口返回数据处理。 这里return的res可在每个组件的全局或动态数据方法中获取</p><h4 id="beforefetch">beforeFetch</h4><p>获取数据发送接口请求前方法，可用于对请求的数据进行处理转换等操作，以提交符合接口的数据要求</p><pre class="language-javascript"><code class="hljs">opt=<span class="hljs-function">(<span class="hljs-params">data, route</span>) =&gt;</span> {
  <span class="hljs-comment">// data请求参数，route当前路由信息</span>
  <span class="hljs-variable language_">console</span>.<span class="hljs-title function_">log</span>(<span class="hljs-string">&#39;beforeFetch&#39;</span>,data)
  <span class="hljs-keyword">return</span> data <span class="hljs-comment">// 这里需将处理后请求数据返回</span>
}</code></pre><h4 id="afterfetch">afterFetch</h4><p>发送数据请求响应事件，这里可对接收到的数据进行处理，以符合使用需要</p><pre class="language-javascript"><code class="hljs">opt=<span class="hljs-function">(<span class="hljs-params">type, res</span>) =&gt;</span> {
<span class="hljs-comment">// 请求响应结果 这里返回的数据在当前页面可使用getScreenGlobal获取</span>
    <span class="hljs-variable language_">console</span>.<span class="hljs-title function_">log</span>(<span class="hljs-string">&#39;afterFetch&#39;</span>,res)
    <span class="hljs-keyword">return</span> res <span class="hljs-comment">// 这里返回的数据在当前页面可使用getScreenGlobal获取</span>
...
}</code></pre><ul><li>2.动态：从<code>url</code>获取，同时在当前<code>afterFetch</code>事件中也可取到全局的数据</li></ul><h4 id="afterfetch-1">afterFetch</h4><p>数据请求结果完成事件，需要此事件对数据进行处理。 使用此方法对组件设置，可满足各种图表数据展示，而无需对<code>echarts</code>进行层层封装，可直接从<code>echarts</code>官网将图表配置好， 复制过来替换好接口数据即可。减少学习成本</p><pre class="language-javascript"><code class="hljs">opt=<span class="hljs-function">(<span class="hljs-params">res, data</span>) =&gt;</span> {
    <span class="hljs-comment">// res响应数据, 当前组件数据data</span>
    <span class="hljs-comment">// 这里可直接使用getScreenGlobal取得全局的数据</span>
    <span class="hljs-variable language_">console</span>.<span class="hljs-title function_">log</span>(<span class="hljs-string">&#39;afterFetchScreen&#39;</span>,data)
...
}</code></pre>`,31),c=[l],h={__name:"screen",setup(p){return(t,o)=>(s(),a("div",e,c))}};export{h as default};
