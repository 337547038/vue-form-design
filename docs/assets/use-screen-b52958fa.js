/* empty css                                                                    */import{o as s,c as a,ai as p}from"./index-3679850f.js";const n={class:"marked-body"},e=p(`<h1 id="可视化数据大屏">可视化数据大屏</h1><h2 id="组件">组件</h2><p> 所支持的设计组件 </p><h2 id="图层">图层</h2><p>　 　大屏可视化设计的所有组件图层，可对当前组件进行隐藏、锁定、删除、选中操作</p><h2 id="大屏配置">大屏配置</h2><p>设计大屏宽高背景色及大屏全局样式</p><h3 id="全局数据">全局数据</h3><p>当可视化大屏引用组件比较多时，若每个组件都单独从接口获取数据，这种做法明显不合理，后端同学可能也不会这么提供。因此引入全局数据请求接口，将数据接口处理好提供给每个组件调用。如可在<code>afterResponse</code>中对接口返回数据处理。这里return的res可在每个组件的全局或动态数据方法中获取</p><pre class="language-javascript"><code class="hljs">opt = <span class="hljs-function">(<span class="hljs-params">res</span>) =&gt;</span>
{
